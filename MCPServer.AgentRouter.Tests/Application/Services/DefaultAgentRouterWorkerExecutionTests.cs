...
public sealed class DefaultAgentRouterWorkerExecutionTests
{
    ...
    [Fact]
    public async Task ExecuteAsync_Uses_Execution_Lease_To_Prevent_Duplicate_Worker_Ownership()
    {
        var plugin = new BlockingAgentPlugin();
        using var container = BuildContainer(plugin);
        var coordinator = container.Resolve<IAgentRunCoordinator>();
        var runStore = container.Resolve<InMemoryAgentRunStore>();
        var traceWriter = container.Resolve<CapturingAgentTraceWriter>();
        var pluginRegistry = container.Resolve<IAgentPluginRegistry>();
        var policyEvaluator = container.Resolve<IAgentPluginPolicyEvaluator>();
        var objective = CreateObjective("only one worker owns this run");
        var request = new AgentRouterStartRunRequest(
            Objective: objective,
            Metadata: new Dictionary<string, string?>
            {
        ...
        var firstExecutor = new DefaultAgentRunExecutor(runStore, traceWriter, pluginRegistry, policyEvaluator);
        var secondExecutor = new DefaultAgentRunExecutor(runStore, traceWriter, pluginRegistry, policyEvaluator);

        var first = firstExecutor.ExecuteAsync(workItem, TestContext.Current.CancellationToken).AsTask();
        await plugin.Started.WaitAsync(TimeSpan.FromSeconds(5), TestContext.Current.CancellationToken);

        var secondFin = await secondExecutor.ExecuteAsync(workItem, TestContext.Current.CancellationToken);
        var second = TestFin.Success<AgentRouterWorkerCycleResult>(secondFin);
        plugin.Release.SetResult();
        var firstFin = await first.WaitAsync(TimeSpan.FromSeconds(5), TestContext.Current.CancellationToken);
        var firstResult = TestFin.Success<AgentRouterWorkerCycleResult>(firstFin);
        var snapshot = TestFin.Success(await runStore.GetSnapshotAsync(start.RunId, TestContext.Current.CancellationToken));

        Assert.Equal(1, plugin.ExecuteCount);
        Assert.Equal(1, second.ProcessedCount);
        Assert.Contains("not leased", second.Message, StringComparison.OrdinalIgnoreCase);
    ...
    private sealed class BlockingAgentPlugin : IAgentPlugin
    {
        ...
        public TaskCompletionSource Release => _release;
            ...
            CancellationToken cancellationToken)
        {
            ...
            return Fin.Succ(AgentPluginExecutionResult.Success("completed", "Blocking plugin completed.", null));
        }
    }
