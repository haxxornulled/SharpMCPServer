...
- For SQLite-backed SSH profile and credential stores, keep connections pooled, set WAL mode and a busy timeout, initialize each schema once per database path, and avoid coarse per-operation serialization. Keep SSH profile settings and credential references in SQLite, do not fall back to environment variables for secrets, and only use the narrowest bootstrap lock such as first-time master-key creation.
- If a vault unlock KDF is ever introduced for SSH-at-rest protection, prefer Argon2id for key derivation. Do not use Argon2 to hash credentials that must later be recovered by SSH execution paths.
...
- For workspace sandboxes, persist the registry in SQLite so stdio and HTTP host instances share the same state. Do not reintroduce per-process in-memory sandbox registries or environment-variable-based workspace expansion. Keep sandbox create/delete approval-gated and workspace-root scoped.
