...
public sealed class McpCapabilityHonestyTests
{
    ...
    public async Task Advertised_Capabilities_Have_Registered_Method_Handlers()
    {
        ...
        AssertCapabilityMethods(capabilities, "tasks", handlers, McpMethods.TasksList, McpMethods.TasksGet, McpMethods.TasksResult, McpMethods.TasksCancel);
    ...
    public async Task Advertised_ListChanged_Flags_Are_Not_Set_Without_Notification_Emitters()
    {
        ...
        var tasks = capabilities.GetProperty("tasks");
        Assert.Equal(JsonValueKind.Object, tasks.GetProperty("list").ValueKind);
        Assert.Equal(JsonValueKind.Object, tasks.GetProperty("cancel").ValueKind);
    }
