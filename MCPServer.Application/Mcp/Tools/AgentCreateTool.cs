...
public sealed class AgentCreateTool : IMcpTool
{
    private static readonly JsonElement InputSchema = AgentToolSchemas.CreateCreateInputSchema();
    ...
    public AgentCreateTool(IAgentRunCoordinator coordinator)
    {
    ...
    {
        Name = AgentToolNames.Create,
        Title = "Create Agent Run",
        Description = "Creates a new agent run with the required capability metadata.",
    ...
    public async ValueTask<Fin<ToolCallResult>> ExecuteAsync(JsonElement? arguments, CancellationToken cancellationToken)
    {
        ...
        var requestResult = AgentToolArguments.Parse(arguments, AgentRouterToolJsonSerializerContext.Default.AgentRunCreateRequest, AgentToolNames.Create);
        ...
        var startRequestResult = AgentToolHelpers.CreateStartRequest(request, isSubagent: false, AgentToolNames.Create);
        ...
        return await RenderSnapshotAsync(runId, cancellationToken, $"Created agent run '{runId.Value}'.").ConfigureAwait(false);
    }
    ...
    {
        var snapshotResult = await AgentToolHelpers.GetSnapshotAsync(_coordinator, runId, AgentToolNames.Create, cancellationToken).ConfigureAwait(false);
