...
public sealed class PromptsListHandler : IMcpMethodHandler
{
    ...
    public PromptsListHandler(IMcpPromptRegistry promptRegistry)
    {
        ArgumentNullException.ThrowIfNull(promptRegistry);
        _promptRegistry = promptRegistry;
    }

    public string Method => McpMethods.PromptsList;
    ...
    public ValueTask<Fin<JsonElement>> HandleAsync(JsonElement? parameters, CancellationToken cancellationToken)
    {
        ...
        {
            return Fail(cursorRead.Error is { } cursorError ? cursorError.Message : "prompts/list cursor could not be read.");
        }

        var outcome = _promptRegistry.ListPrompts(cursorRead.Cursor).Match(
        ...
        {
            return new ValueTask<Fin<JsonElement>>(Fin.Fail<JsonElement>(outcome.Error ?? Error.New("prompts/list failed.")));
        }

        var payload = JsonSerializer.SerializeToElement(result, McpJsonSerializerContext.Default.PromptsListResult);
    ...
    private static CursorReadResult ReadCursor(JsonElement? parameters)
    {
        ...
        {
            return CursorReadResult.Fail(Error.New("prompts/list parameters must be an object when supplied."));
        }
        ...
        {
            return CursorReadResult.Fail(Error.New($"prompts/list parameters are invalid JSON: {ex.Message}"));
        }
    }
    ...
    {
        private ListOutcome(PromptsListResult? result, Error? error, bool isSuccess)
        {
            Result = result;
            Error = error;
            IsSuccess = isSuccess;
        ...
        public PromptsListResult? Result { get; }

        public Error? Error { get; }

        public bool IsSuccess { get; }

        public static ListOutcome Success(PromptsListResult result)
        {
