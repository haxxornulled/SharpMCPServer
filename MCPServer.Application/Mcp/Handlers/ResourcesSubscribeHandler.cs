...
public sealed class ResourcesSubscribeHandler : IMcpMethodHandler
{
    private readonly IMcpResourceSubscriptionRegistry _subscriptions;

    public ResourcesSubscribeHandler(IMcpResourceSubscriptionRegistry subscriptions)
    {
        ArgumentNullException.ThrowIfNull(subscriptions);
        _subscriptions = subscriptions;
    }

    public string Method => McpMethods.ResourcesSubscribe;
    ...
    public ValueTask<Fin<JsonElement>> HandleAsync(JsonElement? parameters, CancellationToken cancellationToken)
    {
        ...
        {
            return Fail(read.Error is { } readError ? readError.Message : "resources/subscribe parameters are invalid.");
        }

        var outcome = _subscriptions.Subscribe(request.Uri).Match(
        ...
        {
            return new ValueTask<Fin<JsonElement>>(Fin.Fail<JsonElement>(outcome.Error ?? Error.New("resources/subscribe failed.")));
        }
    ...
    private static RequestReadResult ReadRequest(JsonElement? parameters)
    {
        ...
        {
            return RequestReadResult.Fail(Error.New("resources/subscribe parameters are required and must be an object."));
        }
        ...
        {
            return RequestReadResult.Fail(Error.New($"resources/subscribe parameters are invalid JSON: {ex.Message}"));
        }

        return request is { Uri: { Length: > 0 } }
            ? RequestReadResult.Success(request)
            : RequestReadResult.Fail(Error.New("resources/subscribe requires a non-empty uri."));
    }
