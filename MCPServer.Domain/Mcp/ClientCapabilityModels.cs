...
public sealed class McpClientCapabilityState
{
    ...
    public bool ElicitationSupportsForm { get; init; }

    public bool ElicitationSupportsUrl { get; init; }
    ...
    public bool TasksSupportsSamplingCreateMessage { get; init; }

    public bool TasksSupportsElicitationCreate { get; init; }
    ...
    public static bool TryRead(JsonElement capabilities, out McpClientCapabilityState state, out string error)
    {
        ...
        var elicitationSupportsForm = false;
        var elicitationSupportsUrl = false;
        if (capabilities.TryGetProperty("elicitation"u8, out var elicitationElement))
        {
            ...
            elicitationSupportsForm = HasObjectProperty(elicitationElement, "form"u8, "Client elicitation.form", out error);
            if (error.Length != 0)
            {
                return false;
            }

            elicitationSupportsUrl = HasObjectProperty(elicitationElement, "url"u8, "Client elicitation.url", out error);
            if (error.Length != 0)
            {
                return false;
            }
        ...
        var tasksSupportsSamplingCreateMessage = false;
        var tasksSupportsElicitationCreate = false;
        if (capabilities.TryGetProperty("tasks"u8, out var tasksElement))
        {
            ...
            if (tasksElement.TryGetProperty("requests"u8, out var taskRequestsElement))
            {
                if (taskRequestsElement is not { ValueKind: JsonValueKind.Object })
                {
                    error = "Client tasks.requests capability must be a JSON object.";
                    return false;
                }

                if (taskRequestsElement.TryGetProperty("sampling"u8, out var samplingRequestsElement))
                {
                    if (samplingRequestsElement is not { ValueKind: JsonValueKind.Object })
                    {
                        error = "Client tasks.requests.sampling must be a JSON object when supplied.";
                        return false;
                    }

                    tasksSupportsSamplingCreateMessage = HasObjectProperty(samplingRequestsElement, "createMessage"u8, "Client tasks.requests.sampling.createMessage", out error);
                    if (error.Length != 0)
                    {
                        return false;
                    }
                ...
                if (taskRequestsElement.TryGetProperty("elicitation"u8, out var elicitationRequestsElement))
                {
                    if (elicitationRequestsElement is not { ValueKind: JsonValueKind.Object })
                    {
                        error = "Client tasks.requests.elicitation must be a JSON object when supplied.";
                        return false;
                    }

                    tasksSupportsElicitationCreate = HasObjectProperty(elicitationRequestsElement, "create"u8, "Client tasks.requests.elicitation.create", out error);
                    if (error.Length != 0)
                    {
                        return false;
                    }
        ...
        state = new McpClientCapabilityState
        {
            ...
            ElicitationSupportsForm = elicitationSupportsForm,
            ElicitationSupportsUrl = elicitationSupportsUrl,
            SupportsTasks = supportsTasks,
            TasksSupportsList = tasksSupportsList,
            TasksSupportsCancel = tasksSupportsCancel,
            TasksSupportsSamplingCreateMessage = tasksSupportsSamplingCreateMessage,
            TasksSupportsElicitationCreate = tasksSupportsElicitationCreate
        };
