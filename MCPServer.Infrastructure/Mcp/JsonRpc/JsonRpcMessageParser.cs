...
public sealed class JsonRpcMessageParser : IJsonRpcMessageParser
{
    ...
    public Fin<JsonRpcMessage> Parse(ReadOnlyMemory<byte> json)
    {
        ...
        try
        {
            ...
            {
                return Fail("JSON-RPC batch messages are not supported in Phase 1.");
            }
            ...
            return Fin.Succ<JsonRpcMessage>(new JsonRpcMessage(jsonRpc, method, id, parameters, hasResult, hasError));
        }
