...
public sealed class AgentRun
{
    ...
    private AgentRun(
        ...
        AgentExecutionLease? executionLease)
    {
        if (runId.IsEmpty)
        {
        ...
        if (executionLease is { ExpiresAtUtc: var expiresAtUtc, AcquiredAtUtc: var acquiredAtUtc }
            && expiresAtUtc <= acquiredAtUtc)
        {
            throw new ArgumentOutOfRangeException(
                nameof(executionLease),
                "Agent execution lease expiration must be later than acquisition time.");
        }
        ...
        ExecutionLease = executionLease is { IsEmpty: false } lease ? lease : null;
    }
    ...
    public AgentExecutionLease? ExecutionLease { get; private set; }
    ...
    public AgentRunTransitionResult AcquireExecutionLease(
        string ownerId,
        DateTimeOffset nowUtc,
        TimeSpan duration,
        string? message)
    {
        ...
        {
            return AgentRunTransitionResult.Rejected(
                AgentRunTransitionErrorCodes.InvalidTransition,
                "Agent run lease acquisition timestamp cannot be earlier than the run creation timestamp.");
        }
        ...
        if (ExecutionLease is { } currentLease
            && !currentLease.IsExpiredAt(nowUtc)
            && !currentLease.IsOwnedBy(ownerId))
        {
            return AgentRunTransitionResult.Rejected(
                AgentRunTransitionErrorCodes.LeaseAlreadyHeld,
                $"Agent run '{RunId.Value}' is already leased by worker '{currentLease.OwnerId}' until '{currentLease.ExpiresAtUtc:O}'.");
        }

        ExecutionLease = AgentExecutionLease.Acquire(ownerId, nowUtc, duration);
        UpdatedAtUtc = nowUtc;
        Message = message;
        Version++;
        ...
        string? message)
    {
        ...
        if (ShouldClearExecutionLease(nextStatus))
        {
            ExecutionLease = null;
        }
    ...
    private static bool ShouldClearExecutionLease(string nextStatus)
    {
        return nextStatus is AgentRunStatuses.Queued
            or AgentRunStatuses.AwaitingApproval
            or AgentRunStatuses.InputRequired
            || AgentRunStatusRules.IsTerminal(nextStatus);
    }
