...
| stdio transport | Implemented | stdio remains the default host transport and supports outbound client-feature requests, task status notifications, and graceful shutdown. |
| Streamable HTTP transport | Implemented | Loopback binding, `Origin` validation, `MCP-Protocol-Version`, `MCP-Session-Id`, `Last-Event-ID`, resumable SSE, `DELETE` teardown, and bounded replay are implemented. |
...
| Server features | Implemented | Tools, resources, prompts, logging, completions, and tasks are exposed by the host. |
