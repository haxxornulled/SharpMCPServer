...
public sealed class SqliteAgentRunStore : IAgentRunStore
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var leaseOwnerId = snapshot.ExecutionLease?.OwnerId;
            var leaseAcquiredAtUtc = snapshot.ExecutionLease?.AcquiredAtUtc;
            var leaseExpiresAtUtc = snapshot.ExecutionLease?.ExpiresAtUtc;
            var rows = await dbContext.AgentRuns
                ...
                .ExecuteUpdateAsync(setters => setters
                    ...
                    .SetProperty(run => run.LeaseOwnerId, leaseOwnerId)
                    .SetProperty(run => run.LeaseAcquiredAtUtc, leaseAcquiredAtUtc)
