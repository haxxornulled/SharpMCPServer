...
public sealed class SshNetCommandExecutor : ISshCommandExecutor
{
    ...
    public SshNetCommandExecutor(ISshPathResolver pathResolver, ILogger<SshNetCommandExecutor> logger)
    {
    ...
    private IReadOnlyList<SshConnectionAttempt> CreateConnectionAttempts(SshExecutionCommand command)
    {
        ...
        if (!string.IsNullOrWhiteSpace(command.PrivateKeyPath))
        {
            ...
            var passphrase = ReadOptionalEnvironmentVariable(command.PrivateKeyPassphraseEnvironmentVariable);
        ...
        var password = ReadOptionalEnvironmentVariable(command.PasswordEnvironmentVariable);
        ...
        if (attempts.Count == 0)
        {
            throw new InvalidOperationException($"SSH profile '{command.ProfileName}' must configure privateKeyPath or passwordEnvironmentVariable. Raw credentials are not accepted in tool requests.");
        }
