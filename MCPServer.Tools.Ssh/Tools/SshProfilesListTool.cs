...
public sealed class SshProfilesListTool : IMcpTool
{
    ...
    public SshProfilesListTool(ISshProfileStore profileStore)
    {
    ...
    public async ValueTask<Fin<ToolCallResult>> ExecuteAsync(JsonElement? arguments, CancellationToken cancellationToken)
    {
        ...
        var structuredContent = SshJson.ToProfilesJsonElement(catalog);
        ...
        using var document = JsonDocument.Parse("""
        {
          ...
          "properties": {
            "profiles": {
              ...
              "items": {
                "type": "object",
                "required": ["name", "host", "port", "username", "hostKeyPinned", "acceptUnknownHostKey", "allowAllCommands", "privileged", "allowedRoot", "allowedCommands", "allowedRemotePathPrefixes", "source"],
                "properties": {
                  "name": { "type": "string" },
                  "host": { "type": "string" },
                  "port": { "type": "integer" },
