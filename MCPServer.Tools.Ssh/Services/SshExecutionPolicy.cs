...
public sealed class SshExecutionPolicy : ISshExecutionPolicy
{
    ...
    public async ValueTask<Fin<SshExecutionPolicyDecision>> EvaluateAsync(SshExecutionRequest request, CancellationToken cancellationToken)
    {
        ...
        var timeoutSeconds = request.TimeoutSeconds is null
            ...
            : Math.Clamp(request.TimeoutSeconds.Value, 1, Math.Max(1, settings.TimeoutSeconds));
    ...
    private static bool IsShellInterpreter(string normalizedCommand)
    {
        return normalizedCommand.Equals("bash", StringComparison.OrdinalIgnoreCase) ||
               ...
               normalizedCommand.Equals("zsh", StringComparison.OrdinalIgnoreCase) ||
               normalizedCommand.Equals("fish", StringComparison.OrdinalIgnoreCase) ||
